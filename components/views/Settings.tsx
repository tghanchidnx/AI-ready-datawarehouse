...
import { MOCK_ENVIRONMENTS, MOCK_DB_CONNECTIONS, DB_CONNECTION_METADATA, MOCK_ROLES, MOCK_PERMISSIONS, MOCK_ACCESS_REVIEWS, MOCK_VENDORS, MOCK_DR_PLAN, MOCK_LLM_PROVIDER_CONFIGS, MOCK_AGENT_CONFIGS, MOCK_WORKFLOWS } from '../../constants';
import type { EnvironmentConfig, DatabaseConnection, Role, Permission, AccessReview, Vendor, LLMProviderConfig, AgentConfig, LLMProviderType, Workflow } from '../../types';
import { DatabaseConnectionType } from '../../types';
import { PlusCircle, Edit, Trash2, Plug, BookOpen, Youtube, ShieldCheck, FileClock, Users, KeyRound, CloudCog, Bot, BrainCircuit, Settings as SettingsIcon, Database as DatabaseIcon, Shield as ShieldIcon, Lock, Landmark, Projector } from 'lucide-react';
...
const Settings: React.FC = () => {
  ...
  const tabs = [
    ...
    { name: 'Agents', icon: <BrainCircuit size={18} /> },
    { name: 'Workflows', icon: <Projector size={18} /> },
    { name: 'Connections', icon: <DatabaseIcon size={18} /> },
    { name: 'Access Control', icon: <Lock size={18} /> },
    { name: 'Compliance', icon: <Landmark size={18} /> },
  ];
  ...
  const StatusPill: React.FC<{ status: DatabaseConnection['status'] }> = ({ status }) => {
    const statusMap = { 'Connected': 'bg-green-500/20 text-green-300', 'Disconnected': 'bg-red-500/20 text-red-300', 'Testing...': 'bg-blue-500/20 text-blue-300 animate-pulse' };
      ...
      <div className="space-y-8">
        ...
        {activeTab === 'Agents' && (<>
            <Card title="LLM Provider Configurations">
              <div className="flex justify-end mb-4"><button onClick={() => handleOpenLlmModal(null)} className="flex items-center gap-2 px-3 py-2 text-sm bg-brand-accent text-white rounded-lg hover:bg-blue-500 transition-colors font-semibold"><PlusCircle size={18} /><span>Add Provider</span></button></div>
            ...
            <Card title="Agent Configurations">
              <div className="flex justify-end mb-4"><button onClick={() => handleOpenAgentModal(null)} className="flex items-center gap-2 px-3 py-2 text-sm bg-brand-accent text-white rounded-lg hover:bg-blue-500 transition-colors font-semibold"><PlusCircle size={18} /><span>Add New Agent</span></button></div>
                <div className="space-y-4">
                    {agents.map(agent => (
                        <div key={agent.id} className="p-4 bg-brand-primary rounded-md border border-brand-border">
                            ...
                            <div className="max-w-xs">
                                ...
                                <select value={agent.llmProviderConfigId || ''} onChange={e => handleAgentChange(agent.id, 'llmProviderConfigId', e.target.value || null)} className="w-full bg-brand-secondary border border-brand-border rounded-md px-3 py-2 text-sm text-white focus:ring-brand-accent focus:border-brand-accent">
        ...
        {activeTab === 'Access Control' && (<>
            <Card title="Role-Based Access Control (RBAC)">
                <div className="space-y-4">{roles.map(role => (<div key={role.id} className="p-4 bg-brand-primary rounded-md border border-brand-border"><h5 className="font-semibold text-white">{role.name}</h5><p className="text-sm text-brand-muted mb-3">{role.description}</p><div className="grid grid-cols-2 gap-x-4 gap-y-2">{MOCK_PERMISSIONS.map(permission => (<label key={permission.id} className="flex items-center space-x-2"><input type="checkbox" className="h-4 w-4 rounded bg-brand-secondary border-brand-border text-brand-accent focus:ring-brand-accent" checked={role.permissions.includes(permission.id)} onChange={(e) => handlePermissionChange(role.id, permission.id, e.target.checked)} /><span className="text-sm text-gray-300">{permission.name}</span></label>))}</div></div>))}</div>
            </Card>
        </>)}
        ...
        {activeTab === 'Compliance' && (<>
            <Card title="SOC 2 Compliance Controls">
                <div className="space-y-6">
                    ...
                    <div><h4 className="flex items-center gap-2 text-lg font-semibold text-brand-accent mb-3"><Users size={20}/><span>Vendor Risk Management</span></h4><div className="p-4 bg-brand-primary rounded-md border border-brand-border"><table className="w-full text-sm text-left"><thead className="text-xs text-brand-muted uppercase"><tr><th className="py-2">Vendor</th><th className="py-2">Risk</th><th className="py-2">Next Review</th><th className="py-2">Status</th></tr></thead><tbody>{vendors.map(vendor => (<tr key={vendor.id} className="border-t border-brand-border"><td className="py-2 font-semibold text-white">{vendor.name} <span className="font-normal text-brand-muted">({vendor.service})</span></td><td className="py-2"><span className={`px-2 py-0.5 text-xs rounded-full ${vendor.risk === 'High' ? 'bg-red-500/20 text-red-300' : 'bg-yellow-500/20 text-yellow-300'}`}>{vendor.risk}</span></td><td className="py-2 text-gray-300">{vendor.nextReview}</td><td className="py-2"><span className={`px-2 py-0.5 text-xs rounded-full ${vendor.status === 'Active' ? 'bg-green-500/20 text-green-300' : 'bg-gray-500/20 text-gray-300'}`}>{vendor.status}</span></td></tr>))}</tbody></table></div></div>
